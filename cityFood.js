...
// ====== Fetch restaurant-like data from TheMealDB API ======

async function fetchRestaurantsByCity(city) {
  try {
    // For demo purposes, we map cities to cuisines
    const cuisineMap = {
      delhi: 'Indian',
      mumbai: 'Indian',
      chennai: 'Seafood',
    ...
    const cuisine = cuisineMap[city.toLowerCase()] || 'Indian';
    const response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${cuisine}`);
    const data = await response.json();

    if (!data.meals) {
      document.querySelector('.restaurants-container').innerHTML = `<p>No restaurants found for ${city}</p>`;
      return;
    }

    displayRestaurants(data.meals, city);
  } catch (error) {
    console.error("Error fetching restaurants:", error);
  }
}

function displayRestaurants(meals, city) {
  ...
  meals.forEach(meal => {
    ...
    card.innerHTML = `
      ...
      <h3>${meal.strMeal}</h3>
      <p>${city}, India</p>
      <p>⭐ ${Math.floor(Math.random() * 2) + 3}.${Math.floor(Math.random() * 9)} Rating</p>
      <button class="view-btn">View Dishes</button>
    `;

    card.querySelector('.view-btn').addEventListener('click', () => {
      localStorage.setItem('selectedRestaurant', JSON.stringify(meal));
      window.location.href = `restrauntDetail.html?meal=${meal.idMeal}`;
    });

    container.appendChild(card);
  });
}
// ===== When page loads, fetch data based on city =====
document.addEventListener('DOMContentLoaded', () => {
  const params = new URLSearchParams(window.location.search);
  const city = params.get('city') || 'Delhi';
  document.title = `Restaurants in ${city}`;
