...
favBtn.style.color = '#444';
favBtn.style.transition = 'color 0.3s ease';
dishDetails.appendChild(favBtn);

// --- Favorite logic using localStorage ---
let favorites = JSON.parse(localStorage.getItem('favoriteDishes')) || [];

// Preload favorite state
if (favorites.some(f => f.foodName === dish.foodName)) {
...
favBtn.addEventListener('click', () => {
    let favorites = JSON.parse(localStorage.getItem('favoriteDishes')) || [];
    const exists = favorites.find(f => f.foodName === dish.foodName);
    ...
    // ========== FAVORITE DISHES FEATURE ==========
function toggleFavorite(dish) {
    let favorites = JSON.parse(localStorage.getItem('favoriteDishes')) || [];
    const exists = favorites.find(fav => fav.foodName === dish.foodName);

    if (exists) {
        favorites = favorites.filter(fav => fav.foodName !== dish.foodName);
    } else {
        favorites.push(dish);
    }

    localStorage.setItem('favoriteDishes', JSON.stringify(favorites));
    updateFavoriteButton(dish.foodName);
}

function updateFavoriteButton(dishName) {
    const btn = document.querySelector(`[data-fav-btn='${dishName}']`);
    const favorites = JSON.parse(localStorage.getItem('favoriteDishes')) || [];
    const isFav = favorites.some(fav => fav.foodName === dishName);
    btn.textContent = isFav ? '★ Favorite' : '☆ Add Favorite';
    btn.style.color = isFav ? 'gold' : 'black';
}

// Run this after displaying all dishes:
function addFavoriteButtons() {
    ...
    dishCards.forEach(card => {
        ...
        favBtn.classList.add('favorite-btn');
        favBtn.setAttribute('data-fav-btn', dishName);
        favBtn.textContent = '☆ Add Favorite';
        favBtn.style.cursor = 'pointer';
        favBtn.style.border = 'none';
        favBtn.style.background = 'transparent';
        ...
        favBtn.addEventListener('click', () => {
            const selectedDish = selectedRestauraunt.food
                .flatMap(cat => Object.values(cat))
                .flat()
                .find(f => f.foodName === dishName);
            toggleFavorite(selectedDish);
        });

        card.appendChild(favBtn);
        updateFavoriteButton(dishName);
    });
}

// Call addFavoriteButtons() after rendering dishes
